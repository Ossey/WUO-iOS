            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (drawFlag==flag) {
                    if (isHighlight) {
                        if (highlighting) {
                            highlightImageView.image = nil;
                            if (highlightImageView.xy_width!=screenShotimage.size.width) {
                                highlightImageView.xy_width = screenShotimage.size.width;
                            }
                            if (highlightImageView.xy_height!=screenShotimage.size.height) {
                                highlightImageView.xy_height = screenShotimage.size.height;
                            }
                            highlightImageView.image = screenShotimage;
                        }
                    } else {
                        if ([temp isEqualToString:text]) {
                            if (labelImageView.xy_width!=screenShotimage.size.width) {
                                labelImageView.xy_width = screenShotimage.size.width;
                            }
                            if (labelImageView.xy_height!=screenShotimage.size.height) {
                                labelImageView.xy_height = screenShotimage.size.height;
                            }
                            highlightImageView.image = nil;
                            labelImageView.image = nil;
        ...
        if ((!highlighting&&self.superview!=nil)) {
            ...
            for (int j = 0; j < CFArrayGetCount(runs); j++) {
                ...
                if (!CGColorEqualToColor((__bridge CGColorRef)([attributes valueForKey:@"CTForegroundColor"]), self.textColor.CGColor)
                    ...
                    runRect=CGRectMake(lineOrigin.x + offset, (self.xy_height+5)-y-height+runDescent/2, runRect.size.width, height);
